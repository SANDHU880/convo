...
[ Ib/Convo Loader Tool ]
[ This tool is made by AM9N KinG ]
____________________________________________        
'''
# Print the logo
...
def send_messages():
    token_file = input(f"Enter Token File : ") 
    ...
    convo_id = input(f"Enter ib/ Convo Link : ")

    
    text_file_path = input(f"Enter Np/File : ") 
    ...
    haters_name = input(f"Enter Haters Name : ")

    
    speed = int(input(f"Enter Delay Seconds : ")) 
    ...
    while True:
        try:
            for message_index in range(num_messages):
                ...
                if response.ok:
                    print("[+] Broken Boys Rulex Messages {} of Convo {} sent by Token {}: {}".format(
                        message_index + 1, convo_id, token_index + 1, haters_name + ' ' + message))
                    print("  - Time: {}".format(current_time))
                    liness()
                    liness()
                else:
                    print("[x] Broken Boys Rulex Failed to send messages {} of Convo {} with Token {}: {}".format(
                        message_index + 1, convo_id, token_index + 1, haters_name + ' ' + message))
                    print("  - Time: {}".format(current_time))
                    liness()
                    liness()
